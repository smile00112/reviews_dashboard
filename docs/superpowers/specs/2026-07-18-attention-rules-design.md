...
severity (urgent → warn → info), внутри severity — по модулю `value` убыв.
(у rating_drop value отрицательный)
...
Клиентский компонент (`"use client"`, как `/jobs`), загружающий данные через
`lib/api.ts`; таблица и форма — компоненты под `components/attention-rules/`:
